    ...
    <form name="fileform" id="fileform" action="{{url('uploadfile')}}" method="post" enctype="multipart/form-data" >
        ...
        <div class="shadow sm:rounded-md sm:overflow-hidden">
          ...
          <div class="col-span-6 sm:col-span-3">
                ...
                <input type="text" name="name" placeholder="curriculum vitae proffesionnel" id="name" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
          </div>
...
<div>
  <div class="md:grid md:grid-cols-3 md:gap-6">
    ...
    <div class="mt-5 md:mt-0 md:col-span-2">
    <form name="demandform"  id="demandform" action="{{url('demande')}}" method="post" enctype="multipart/form-data">
        @csrf 
        <div class="shadow sm:rounded-md sm:overflow-hidden">
          <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
            <div>
              ...
              <div class="mt-1">
                <textarea name="description" id="description" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md" placeholder="correction et mise en forme du cv"></textarea>
              </div>
      ...
      <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
            <button type="button" value="button" onclick="submitForms()"   class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
              Soumettre
            </button>
    </div>
...
<script>
    ...
    var submitForms = document.createElement('button'); 
    submitForms.onclick = function(){ 
        fileform.submit(); 
        shapeform.submit();
        demandform.submit(); 
} 
document.body.appendChild(submitForms);
</script>
